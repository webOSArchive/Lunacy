...
**Adding a patch:** edit `android/local-assets/fw/mojo/`, then diff it against
`spike/vendor/touchpad/mojo/` into `patches/`, the same way the Enyo fork works
(`android/tools/make-enyo-patch.sh` does it there); `fetch-assets.sh` applies them in order
