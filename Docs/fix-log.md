...
| 2026-09-21 | Shift-lock capitalises the letters but leaves the number row typing numbers, and never lets go | LunaKeyboard, in any app | Lunacy's own, and the first pass had it wrong: read off LunaCE's `isShiftActive`, which excludes `eShiftMode_CapsLock`, the number row went on typing digits while locked. **codepoet, who used the device: shift-lock is shift for the number row, and one numeric entry turns it off - it only survives alphabetic keystrokes.** The source reads the other way, so this follows the device. Typed on the tablet, locked: `q w 1 q e` gives `QW!qe` | LunaKeyboard (a lock ends on the first key that takes its alternate; `adb` can't tap twice inside the 500 ms double-tap window, so this was measured with that window temporarily widened) |
