...
| 2026-09-22 | The card's background comes out as flat blocks with hard edges between them, where the device draws one smooth gradient - codepoet: "a weird sliced up blob" | webOS IAmA reddit (any app with a viewport-sized fixed background; Mojo gives every body `min-height: 480px`, so a page whose content doesn't fill the card has a short body) | With `background-attachment: fixed` the background positioning area is the viewport, so a percentage `background-size` is a percentage of the viewport. This WebView resolves it against the element's own box instead - measured: a body made 2772 px tall stretched the image over 2772 rather than the card's 772 - and then tiles what is left, every 264 px. reddit's body carries the app's gradient at `background-size: 100% 100%` with `background-attachment: fixed` | compat (a fixed background with a percentage size is given the viewport's size in pixels, recomputed on resize; one that covers the viewport is also made `no-repeat`, which is the only way to stop the tiling and cannot change a background that covers). Against the reference TouchPad, down the same column: 104 -> 23 here, 102 -> 23 there |
