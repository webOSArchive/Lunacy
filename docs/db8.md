...
kept in memory. Lunacy's is `android/app/src/main/java/org/webosarchive/lunacy/card/Db8.kt`,
on SQLite. Everything below was measured on the reference TouchPad (webOS CE 3.1.0) with
`spike/probe/db8probe.sh` and `db8watch.sh`, run there as the probe app. The same calls run
in Lunacy with `spike/probe/db8-lunacy.js`, and all 41 of them answer as the TouchPad did,
