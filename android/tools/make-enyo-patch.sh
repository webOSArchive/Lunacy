...
out=framework/enyo-1.0/patches/$name.patch
base=$(mktemp -d)
trap 'rm -rf "$base"' EXIT
cp -r ../spike/vendor/enyo-1.0/. "$base/"
for p in framework/enyo-1.0/patches/*.patch; do
    [ -e "$p" ] || continue
    ...
    (cd "$base" && patch -p1 --forward --silent < "$OLDPWD/$p")
done
: > "$out"
for f in "$@"; do
